...
Every channel normalizes platform-specific input into `IncomingMessage` before the agent runs, and receives a shared `OutgoingMessage` after orchestration. Channels translate those common models to and from their own platform SDKs.

Important incoming fields:

- `ChannelId`: concrete channel package or instance, such as `telegram` or `console`
- `ChannelType`: shared transport kind, such as `Chat` or `Console`
- `Text`: normalized user text or caption
- `Attachments`: normalized files such as images, audio, or documents
- `Metadata`: channel-specific identifiers that stay out of the agent contract

Important outgoing fields:

- `Text`: normalized response text
- `ContentStream`: optional streaming chunks for channels that support progressive delivery
- `Attachments`: normalized files generated by the agent
- `Actions`: normalized actions such as buttons, links, or commands
- `Metadata`: response hints that stay independent from channel SDK types
- `ChannelId` and `ChannelType`: copied from the incoming message so the target channel can deliver the response

Agents and middleware should depend on `IncomingMessage` and `OutgoingMessage`, not on Telegram, Console, Slack, or other channel SDK types.
...
- release tags like `v0.1.7`: exact stable package version `0.1.7`
